...
<div class="product">
    @if (count($products) > 0)
        <div class="title">{{__('Pekerja')}}</div>
        <ul>
                                ...
                                <div class="clearfix">
                                    ...
                                    {{-- <div class="stock-box float-right">
                                        ...
                                        @if(count(json_decode($product->variations, true)) >= 1)
                                            @if ($qty > 0)
                                                <span class="badge badge-pill bg-green">{{__('Bersedia')}}</span>
                                            @else
                                                <span class="badge badge badge-pill bg-red">{{__('Tidak bersedia')}}</span>
                                            @endif
                                        @endif
                                    </div> --}}
                                </div>
...
@if(\App\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1)
    <div class="product">
        @auth
            @if (count($shops) > 0)
                <div class="title">{{__('Penyalur')}}</div>
                <ul>
        ...
        @else
            ...
            <ul>
                <li>
                    <a href="#">
                        <div class="d-flex search-product align-items-center">
                            Maaf Untuk melihat Pencarian Penyalur anda harus Mendaftar/Login Akun..
                        </div>
                    </a>
                </li>
            </ul>
