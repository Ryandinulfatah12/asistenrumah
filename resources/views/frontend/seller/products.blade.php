...
@section('meta_title','Data Pekerja')
@section('content')
                        ...
                        <div class="row">
                            <div class="col-md-4 offset-md-4">
                                <a class="dashboard-widget text-center plus-widget mt-4 d-block" href="{{ route('seller.products.upload')}}">
                                    ...
                                    <span class="d-block title heading-6 strong-400 c-base-1">{{ __('Tambah data pekerja') }}</span>
                                </a>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        function update_featured(el){
            ...
            $.post('{{ route('products.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showFrontendAlert('success', 'Pekerja unggulan diperbarui');
                }
                else{
        ...
        function update_published(el){
            ...
            $.post('{{ route('products.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showFrontendAlert('success', 'Status publikasi berhasil diubah!');
                }
                else{
                    showFrontendAlert('danger', 'Something went wrong');
                }
