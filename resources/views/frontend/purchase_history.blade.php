...
@section('meta_title','Riwayat Pesanan')
@section('content')
                                ...
                                <div class="col-md-6 col-12">
                                    <div class="float-md-right">
                                        <ul class="breadcrumb">
                                            ...
                                            <li class="active"><a href="{{ route('purchase_history.index') }}">{{__('Riwayat Pesanan')}}</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            @foreach ($orders as $key => $order)
                                                @if (count($order->orderDetails) > 0)
                                                    <tr>
                                                        ...
                                                        <td>{{ date('d F Y - h:i', $order->date) }}</td>
                                                        ...
                                                        <td>
                                                            ...
                                                            @if($order->delivery_viewed == 0)
                                                                <span class="ml-2" style="color:green"><strong>({{ __('Dilihat penyalur') }})</strong></span>
                                                            @else
                                                        ...
                                                        <td>
                                                            <span class="badge badge--2 mr-4">
                                                                @if ($order->payment_status == 'paid')
                                                                    <i class="bg-green"></i> {{__('Sudah Bayar')}}
                                                                @else
                                                                    <i class="bg-red"></i> {{__('Belum Membayar')}}
                                                                @endif
                                                        ...
                                                        <td>
                                                            <div class="dropdown">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="">
                                                                    <button onclick="show_purchase_history_details({{ $order->id }})" class="dropdown-item">{{__('Detail Pesanan')}}</button>
