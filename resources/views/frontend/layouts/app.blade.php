...
<title>@yield('meta_title') - AsistenRumah.com</title>
...
<script>
    ...
    function removeFromCart(key){
        $.post('{{ route('cart.removeFromCart') }}', {_token:'{{ csrf_token() }}', key:key}, function(data){
            ...
            showFrontendAlert('success', 'Pesanan telah dibatalkan');
    ...
    function addToWishList(id){
        @if (Auth::check() && (Auth::user()->user_type == 'customer' || Auth::user()->user_type == 'seller'))
            $.post('{{ route('wishlists.store') }}', {_token:'{{ csrf_token() }}', id:id}, function(data){
                if(data != 0){
                    $('#wishlist').html(data);
                    showFrontendAlert('success', 'Pekerja telah ditambahkan ke Wishlist');
                }
                else{
                    showFrontendAlert('warning', 'Harap login terlebih dahulu...');
                }
            });
        @else
            showFrontendAlert('warning', 'Harap login terlebih dahulu...');
